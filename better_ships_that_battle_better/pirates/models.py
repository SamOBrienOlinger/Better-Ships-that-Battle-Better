...
class UserProfile(models.Model):
    ...
    chosen_pirate_queen = models.ForeignKey(
        PirateQueen, on_delete=models.SET_NULL, null=True, blank=True
    )
    bio = models.TextField(
        max_length=500, blank=True,
        help_text="Tell us about your pirate adventures"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class Game(models.Model):
    ...
    def __str__(self) -> str:
        # Django provides get_*_display methods for choice fields and
        # related field access at runtime
        date_str = self.played_at.strftime('%Y-%m-%d')
        return (
            f"{self.player.pirate_name} - "
            f"{self.get_result_display()} ({date_str})"
